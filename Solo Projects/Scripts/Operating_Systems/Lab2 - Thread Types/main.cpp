...
void Print() {
	...
	printf("START: Joinable Thread %d, starting limit = %d\n", threadData.id, threadData.promisedWork.get_future().get());
...
void JoinableThreadEntrypoint(ThreadStruct* threadData)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	std::thread printerThread(Print);
	...
	///////////////////////////////////////////////////////////////////////////////////
	threadData->promisedWork.set_value(work);
}
...
void DetachedThreadEntrypoint(ThreadStruct* threadData)
{
	...
	while (true)
	{
		...
		///////////////////////////////////////////////////////////////////////////////////
		if (threadData->detatchFlag == false)
		{
			std::unique_lock<std::mutex> flagLock;
			threadData->cvPtr->wait(flagLock, [&] {return threadData->detatchFlag == false; });
			break;
		}
		// Performs some arbitrary amount of work.
		for (int i = 0; i < workLimit; i += (threadData->id + 1))
	...
	threadData->cvPtr->wait(printLock);
	///////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char** argv)
{
	...
	for (int i = totalThreadCount - 1; i >= 0; i--)
	{
		...
		perThreadData[i].mutexPtr = &countMutex;

		if (i % 2 == 0) {
			threadArray.emplace_back(JoinableThreadEntrypoint, &perThreadData[i]);//even #'s
		}
		else {
			std::thread(DetachedThreadEntrypoint, &perThreadData[i]).detach();//odd #'s
		}
	}
	...
	///////////////////////////////////////////////////////////////////////////////////
	perThreadData->mutexPtr->lock();
	perThreadData->detatchFlag = false;
	perThreadData->cvPtr->notify_one();
	perThreadData->mutexPtr->unlock();
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> temp(countMutex);
	cv.wait(temp, [&] {return !totalThreadCount; });
