...
bool TryToGetResources(Drinker* currentDrinker)
{
	...
	for (int i = 0; i < currentDrinker->resourcePool->totalResources;)
	{
		Resource* secondResource = &currentDrinker->resourcePool->resources[i];
		secondResource->resourceMutex.try_lock();
		if (secondResource->resourceMutex.try_lock() == true)
		{
...
void StartDrinker(Drinker* currentDrinker)
{
	...
	currentDrinker->drinkerPool->stopDrinkingMutex.lock();
	while (true)
	{
	...
	currentDrinker->drinkerPool->stopDrinkingMutex.unlock();
}
...
void DrinkerThreadEntrypoint(Drinker* currentDrinker)
{
	...
	currentDrinker->drinkerPool->drinkerCountCondition.wait(entryLock);


	//unique lock and then wait
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < drinkerCount; i++)
	{
		std::thread drinkers(DrinkerThreadEntrypoint, poolOfDrinkers);
		drinkers.detach();
	}
	//amount = drinker count
	...
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> drinkerLock;
	for (int i = 0; i < drinkerCount; i++)
	{
		poolOfDrinkers.drinkerCountCondition.wait(drinkerLock, [&] {return i == drinkerCount; });
	}
	//condition variables
	...
	std::unique_lock<std::mutex> drinkerSLock;
	poolOfDrinkers.startingGunFlag = true;
	poolOfDrinkers.startingGunCondition.notify_all();
	poolOfDrinkers.startingGunCondition.wait(drinkerSLock, [&] {return poolOfDrinkers.startingGunFlag == true; });
	...
	poolOfDrinkers.stopDrinkingCondition.notify_all();
	poolOfDrinkers.stopDrinkingCondition.wait(stopLock, [&] {return poolOfDrinkers.stopDrinkingFlag == true; });
	//stop drinking flag must be true
	//lock and unlock here
	...
	poolOfDrinkers.drinkerCountCondition.wait(finishLock, poolOfDrinkers.drinkerCount == 0);
