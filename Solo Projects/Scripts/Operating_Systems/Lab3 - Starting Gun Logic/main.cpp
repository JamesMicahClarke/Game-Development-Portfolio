...
struct PlayerPool
{
	...
	std::unique_lock<std::mutex> playerLock;
};
...
void LogSync(LogSyncOperation operationToPerform)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	LogSync(LogSyncOperation::Lock);
	//static only stays inside the scope
	//do no replace printf for board or different kind of printf ex: vprintf
	LogSync(LogSyncOperation::Unlock);
}
...
void PlayGame(Player* currentPlayer, Game* currentGame)
{
	...
	while (currentGame->currentGameState == GameState::StillPlaying)
	{
		...
		case GameState::StillPlaying:
			...
			///////////////////////////////////////////////////////////////////////////////////
			currentPlayer->playerPool->cv.notify_all();
...
void JoinGame(Player* currentPlayer, Game* currentGame)
{
	...
	if (currentGame->playerO == -1)
	{
		...
		currentGame->gameCondition.wait(gameUniqueLock, [&] {return (currentGame->playerCount > 1); });//check
	}
	else
	{
...
void PlayerThreadEntrypoint(Player* currentPlayer)
{
	...
	currentPlayer->playerPool->cv.notify_one();//check
	std::unique_lock<std::mutex> counterLock(currentPlayer->playerPool->lock);
	currentPlayer->playerPool->cv.wait(counterLock, [&] {return (currentPlayer->playerPool->totalPlayerCount < 1); });//check
...
// Attempt to play each game, all of the game logic will occur in this function
	...
	currentPlayer->playerPool->cv.notify_all();//check
}
...
int main(int argc, char** argv)
{
	...
	{
		std::thread t(PlayerThreadEntrypoint, perPlayerData);
		t.detach();
	...
	std::unique_lock<std::mutex> ul(perPlayerData->playerPool->lock);
	for (int i = 0; i < totalPlayerCount; i++)
	{
		perGameData->gameCondition.wait(ul, [&] {return (i == totalPlayerCount); });//check
	...
	///////////////////////////////////////////////////////////////////////////////////
	//ul.lock();
	poolOfGames.perGameData->gameCondition.wait(ul, [&] {return (totalPlayerCount == 0); });//check
	...
	delete[] perGameData->gameUniqueLock;
	delete[] perGameData;
	delete[] perPlayerData->gamePool->perGameData;
	delete[] perPlayerData->gamePool;
	delete[] perPlayerData->playerPool->perPlayerData;
	delete[] perPlayerData->playerPool;
