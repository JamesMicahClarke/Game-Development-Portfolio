...
namespace	// Global to this file only
{
	...
	PowerUp* CreatePowerUp()
	{
		...
		{
			power->SetTexture("Resources/images/pupweapon.png");
		}
...
void LevelState::CheckCollision()
{
	...
	for (unsigned cur = 0; cur < enemies.size(); cur++)
	{
		if (enemies[cur]->GetShip() && enemies[cur]->GetShip()->GetActive())
		{
			...
			if (playerShip)
			{
				if (enemies[cur]->GetShip()->GetActive() && enemies[cur]->GetShip()->Collide(*playerShip))
				{
...
void LevelState::GeneratePowerUps(Enemy* _enemy)
{
	...
	_enemy->GetPowerUpID();
	_enemy->GetMinDistance();
	_enemy->GetMaxDistance();
...
void LevelState::Init()
{
	...
	// boss
	boss = NULL;
	LevelState::GeneratePowerUps(boss);
